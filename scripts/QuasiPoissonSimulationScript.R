##Quasi-Poisson Regression with variable specdens. 
...
for(i in 1:L){
  ...
  for(j in 3:M){
    ...
    model = glm(y~x + Splines,family = "quasipoisson")
...
write_csv(data.frame(coverage),file=paste0("RawData/",format(Sys.time(), "%d%B%Y-%H:%M-G:"),g,"-C:",c, "-CoverageMatrixQuasiPois.csv"))
write_csv(data.frame(estimates),file=paste0("RawData/",format(Sys.time(), "%d%B%Y-%H:%M-G:"),g,"-C:",c, "-EstimatesMatrixQuasiPois.csv"))
write_csv(data.frame(standardError),file=paste0("RawData/",format(Sys.time(), "%d%B%Y-%H:%M-G:"),g,"-C:",c, "-StandardErrorMatrixQuasiPois.csv"))
write_csv(data.frame(b0estimates),file=paste0("RawData/",format(Sys.time(), "%d%B%Y-%H:%M-G:"),g,"-C:",c, "-b0estimatesMatrixQuasiPois.csv"))
